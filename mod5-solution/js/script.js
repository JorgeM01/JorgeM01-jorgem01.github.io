        ...
        $ajaxUtils.sendGetRequest(
          ...
          function (categorySelected) {
            ...
            for (var i = 0; i < categorySelected.length; i++) {
              myArray.push(categorySelected[i].short_name);
            }

            // Creation of a randon number based on the number of categories.
            let randomNumber = Math.floor(Math.random() * myArray.length);

            document.querySelector("#main-content").innerHTML = insertProperty(
              responseText,
